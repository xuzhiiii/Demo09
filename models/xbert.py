...
class BertModel(BertPreTrainedModel):
	...
	def __init__(self, config, add_pooling_layer=True, add_perturbed=False, eps=0, is_uniformity=True):
	...
	def forward(
		...
		if self.perturbed:
			if self.is_uniformity:
				random_noise = torch.rand_like(embedding_output, device=embedding_output.device)
				embedding_output = embedding_output + torch.sign(embedding_output) * F.normalize(random_noise,
				                                                                                 dim=-1) * self.eps
			else:
				random_noise = torch.randn_like(embedding_output, device=embedding_output.device)
				embedding_output = embedding_output + F.normalize(random_noise, dim=-1) * self.eps
